...
let profile={dom("age"):29,dom("height"):159,weight:68,dom("activity"):1.35,dom("deficit"):400,targets:{cal:1700,p:120,f:55,c:185}};
let day={date:today(),cal:0,p:0,f:0,c:0,dom("steps"):0,cycle:0,dom("burn"):0,workout:{}};
...
async function auth(mode){
 const email=dom("authEmail").value.trim(),pass=dom("authPassword").value;
 if(!email||pass.length<6){dom("authMsg").textContent='Enter a valid email and password (6+ characters).';return}
 let r=mode==='signup'?await sb.auth.signUp({email,password:pass}):await sb.auth.signInWithPassword({email,password:pass});
 if(r.error){dom("authMsg").textContent=r.error.message;return}
...
async function loadCloud(){
 ...
 if(p){profile={...profile,dom("age"):p.age||29,dom("height"):p.height_cm||159,weight:p.starting_weight_kg||68,dom("activity"):p.activity_level==='lightly_active'?1.35:1.2,targets:{cal:p.calorie_target||1700,p:p.protein_target||120,f:p.fat_target||55,c:p.carb_target||185}}}
 else await saveProfile(true);
 const {data:logs}=await sb.from('daily_logs').select('*').eq('user_id',user.id).order('date');
 dailyHistory=(logs||[]).map(x=>({date:x.date,cal:x.calories||0,p:x.protein||0,f:x.fat||0,c:x.carbs||0,dom("burn"):x.exercise_calories||0,dom("steps"):x.steps||0,weight:x.weight_kg}));
...
async function upsertDaily(){
 ...
 await sb.from('daily_logs').upsert({user_id:user.id,date:day.date,calories:day.cal,protein:day.p,fat:day.f,carbs:day.c,exercise_calories:day.burn,dom("steps"):day.steps,walking_minutes:Math.round(day.steps/100),cycling_minutes:day.cycle,workout_completed:Object.keys(day.workout).length>0},{onConflict:'user_id,date'});
}
...
async function saveProfile(silent=false){
 profile.age=+dom("age").value||29;profile.height=+dom("height").value||159;profile.weight=+dom("startWeight").value||68;profile.activity=+dom("activity").value||1.35;profile.deficit=+dom("deficit").value||400;calcTarget();
 if(user) await sb.from('profiles').upsert({id:user.id,dom("age"):profile.age,height_cm:profile.height,starting_weight_kg:profile.weight,activity_level:profile.activity===1.35?'lightly_active':'other',calorie_target:profile.targets.cal,protein_target:profile.targets.p,fat_target:profile.targets.f,carb_target:profile.targets.c});
...
function openPortion(key){
 ...
 if(f[6]==='weight'||f.unit==='100g'){dom("portionId").value=key;dom("portionTitle").textContent=f[1]||f.name;dom("portionBase").textContent='Enter the amount in grams.';dom("portionWeight").value=100;openModal('portionModal')}
 else addFood(f,1,'count');
}
function addByWeight(){const f=foodByKey(dom("portionId").value);const g=Math.max(1,+dom("portionWeight").value||0);closeModals();addFood(f,g,'g')}
function addCustomFood(){
 const f={food_key:'custom_'+Date.now(),name:dom("customName").value.trim(),calories_100g:+dom("customCal").value||0,protein_100g:+dom("customP").value||0,fat_100g:+dom("customF").value||0,carbs_100g:+dom("customC").value||0,unit:'100g'};
...
function renderFoodList(q=''){
 ...
 dom("quickTitle").textContent=query?'Search results':'Quick foods';dom("mealWindow").textContent=query?'':LABEL[win()];
 dom("quickFoods").innerHTML=list.length?list.map(x=>`<button onclick="openPortion('${x[0]}')"><span>${x[1]}</span><b>~${Math.round(x[2])} kcal</b></button>`).join(''):'<div class="card">No local match. Try web lookup below.</div>';
 if(query) dom("webLookup").classList.remove('hidden'); else dom("webLookup").classList.add('hidden');
 if(query) dom("webLookup").innerHTML=`<b>🌐 Web lookup</b><p>Search the web for nutrition for “${escapeHtml(query)}”.</p><button class="secondary" onclick="webSearch('${escapeAttr(query)}')">Find nutrition</button>`;
}
function escapeHtml(s){return s.replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}
function escapeAttr(s){return s.replace(/'/g,"\\'")}
async function webSearch(q){
 // OpenFoodFacts is a public nutrition source. It is used as the first web lookup.
 dom("webLookup").innerHTML='<b>🌐 Looking up nutrition…</b><p>Checking Open Food Facts.</p>';
 try{
  ...
  if(!p){dom("webLookup").innerHTML='<b>No usable nutrition result.</b><p>Use Add custom food and enter the label values.</p>';return}
  ...
  dom("webLookup").innerHTML=`<b>${escapeHtml(webPending.name)}</b><small>${escapeHtml(webPending.brand)}</small><p>${webPending.cal} kcal · ${webPending.p} g protein · ${webPending.f} g fat · ${webPending.c} g carbs / 100 g</p><button class="secondary" onclick="confirmWebFood()">Verify & add</button>`;
 }catch(e){dom("webLookup").innerHTML='<b>Web lookup unavailable.</b><p>Use Add custom food instead.</p>'}
}
async function confirmWebFood(){
 if(!webPending)return;
 dom("webFoodInfo").innerHTML=`<b>${escapeHtml(webPending.name)}</b><p>${webPending.cal} kcal · ${webPending.p} g protein · ${webPending.f} g fat · ${webPending.c} g carbs / 100 g<br>Source: ${webPending.source}</p>`;
 dom("webAmount").value=100;openModal('portionConfirmModal')
}
async function addWebFood(){
 ...
 const g=Math.max(1,+dom("webAmount").value||100);closeModals();addFood(f,g,'g');webPending=null
}
...
function generateMealPlan(){
 ...
 dom("planTitle").textContent=p>40?'Protein-focused next meal':'Balanced next meal';dom("planText").textContent=text
}
...
function schedule(){
 ...
 const slots=[
  ...
  {time:'Evening',icon:'🌇',text:'25–40 min exercise cycle or brisk walk if dom("activity") target is not met.',show:true},
  {time:'Night',icon:'🌙',text:'Optional light mobility. Avoid hard training close to bedtime.',show:true}
 ];
 return slots;
}
function renderWorkout(){
 const targets=workoutTargets();dom("workoutList").innerHTML=targets.map(x=>`<div class="workout-row"><span class="exerciseName">${x[1]}<small>Target ${x[2]} ${x[3]}</small></span><strong class="target">${x[2]}</strong><div class="stepper"><button onclick="changeExercise('${x[0]}',-1)">−</button><b>${day.workout[x[0]]||0}</b><button onclick="changeExercise('${x[0]}',1)">+</button></div></div>`).join('');
 dom("scheduleList").innerHTML=schedule().map(x=>`<div class="slot"><strong>${x.icon} ${x.time}</strong><small>${x.text}</small></div>`).join('');
 dom("workoutWindowTitle").textContent='Smart daily schedule';dom("workoutWindowText").textContent='Strength is best when you are fueled; walking/cardio can be split into shorter sessions.';
}
function saveActivity(){day.cycle=Math.max(0,+dom("cycleInput").value||0);day.burn=Math.max(0,+dom("burnInput").value||0);saveLocal();upsertDaily();render();toast('Activity saved')}
async function saveWeight(){const w=+dom("newWeight").value||0;if(!w)return;day.weight=w;saveLocal();await upsertDaily();if(user)await sb.from('body_measurements').insert({user_id:user.id,date:day.date,weight_kg:w});closeModals();render();toast('Weight saved')}
async function saveMeasurement(){const x={weight_kg:+dom("measureWeight").value,belly_cm:+dom("measureBelly").value,waist_cm:+dom("measureWaist").value,chest_cm:+dom("measureChest").value,biceps_cm:+dom("measureBiceps").value};if(user)await sb.from('body_measurements').insert({user_id:user.id,date:today(),...x});day.weight=x.weight_kg;await upsertDaily();closeModals();render();toast('Measurements saved')}
function simpleChart(el,vals,label,target=0){
 ...
 el.innerHTML=`<div style="font-size:10px;color:#6b7280">${label}</div><svg viewBox="0 0 ${w} ${h}" width="100%" dom("height")="125"><polyline points="${pts}" fill="none" stroke="#111827" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>${target?`<line x1="${p}" x2="${w-p}" y1="${h-p-(target-min)/(max-min)*(h-2*p)}" y2="${h-p-(target-min)/(max-min)*(h-2*p)}" stroke="#9ca3af" stroke-dasharray="5 5"/>`:''}</svg><small>${vals.map(v=>Math.round(v)).join(' · ')}</small>`;
}
async function refreshHistory(){
 if(user){const {data}=await sb.from('daily_logs').select('*').eq('user_id',user.id).order('date');dailyHistory=(data||[]).map(x=>({date:x.date,cal:x.calories||0,p:x.protein||0,f:x.fat||0,c:x.carbs||0,dom("burn"):x.exercise_calories||0,dom("steps"):x.steps||0,weight:x.weight_kg}))}
}
async function analyzeWeek(){
 await refreshHistory();const d=dailyHistory.slice(-7);if(d.length<7){dom("analysis").textContent=`You have ${d.length} logged day(s). Keep going until 7 days before making a meaningful calorie change.`;return}
 ...
 let action=Math.abs(delta)<.1?'Keep calories and improve dom("activity") consistency.':delta<-.8?'Progress may be fast; protect strength and recovery.':'Plan appears reasonable; keep monitoring the belly/waist trend.';
 dom("analysis").innerHTML=`<b>Week review</b><br>Average calories: ${Math.round(avg('cal'))} kcal/day<br>Average protein: ${Math.round(avg('p'))} g/day<br>Exercise dom("burn"): ${Math.round(d.reduce((a,x)=>a+x.burn,0))} kcal/week<br>Average dom("steps"): ${Math.round(avg('steps')).toLocaleString()}<br>Weight change: ${delta>=0?'+':''}${delta.toFixed(1)} kg<br><br><b>Recommendation:</b> ${action}`;
}
function render(){
 const t=profile.targets;
 dom("dayLabel").textContent='Day '+(Math.max(1,dailyHistory.length+1));
 dom("calUsed").textContent=Math.round(day.cal);dom("calTarget").textContent=t.cal;dom("proteinUsed").textContent=`${Math.round(day.p)} / ${t.p} g`;dom("fatUsed").textContent=`${Math.round(day.f)} / ${t.f} g`;dom("carbUsed").textContent=`${Math.round(day.c)} / ${t.c} g`;dom("calBar").style.width=Math.min(100,day.cal/t.cal*100)+'%';
 dom("remainingCal").textContent=Math.max(0,Math.round(t.cal-day.cal)).toLocaleString()+' kcal';dom("remainingP").textContent=Math.max(0,Math.round(t.p-day.p))+' g protein';dom("remainingF").textContent=Math.max(0,Math.round(t.f-day.f))+' g fat';
 dom("steps").textContent=day.steps.toLocaleString();dom("stepRange").value=day.steps;dom("walkMin").textContent=Math.round(day.steps/100)+' min';dom("cycleMin").textContent=day.cycle+' min';dom("burn").textContent=Math.round(day.burn)+' kcal';
 const dom("score")=Math.round(Math.min(35,day.p/t.p*35)+Math.min(35,day.cal<=t.cal?35:t.cal/day.cal*35)+Math.min(15,day.steps/7000*15)+(Object.keys(day.workout).length?15:0));document.getElementById('score').textContent=dom("score");
 coachText.textContent=`Target ${t.cal} kcal · ${t.p} g protein. Split exercise across the day rather than forcing one long session.`;
 dom("profileSummary").textContent=`${profile.age} · ${profile.height} cm · Lightly active · ${t.cal} kcal`;
 renderFoodList(dom("foodSearch")?.value||'');renderMeals();renderWorkout();
 dom("nextActivity").innerHTML=`<strong>🌇 Evening cardio</strong><br>25–40 min cycle or brisk walk if you haven't met today's dom("activity") target.`;
 dom("workoutQuick").textContent=Object.keys(day.workout).length?'Progress saved':'See today's targets';
 simpleChart(dom("nutritionChart"),dailyHistory.slice(-7).map(x=>x.cal),'Calories',t.cal);simpleChart(dom("activityChart"),dailyHistory.slice(-7).map(x=>x.burn),'Exercise calories');simpleChart(dom("weightChart"),dailyHistory.slice(-7).map(x=>x.weight).filter(x=>x!=null),'Weight');
 dom("measurementLog").innerHTML=`<div class="line"><span>Starting</span><b>68 kg · 97 cm belly · 90 cm waist</b></div>`;
}
function renderMeals(){
 if(!meals.length){dom("mealLog").innerHTML='<div class="card">No meals logged today.</div>';return}
 dom("mealLog").innerHTML=meals.map(x=>`<div class="meal"><span>${x.name}${x.unit==='count'?' × '+x.qty:' — '+Math.round(x.qty)+' g'}<small>${Math.round(x.p)} g protein · ${Math.round(x.f)} g fat · ${Math.round(x.c)} g carbs</small></span><b>${Math.round(x.cal)} kcal</b></div>`).join('');
}
